...
```bash
nextflow pull phac-nml/cditoxins
```
...
First, go to the [phac-nml/cditoxins releases page](https://github.com/phac-nml/cditoxins/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
